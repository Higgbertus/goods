...
//                   shipInstance.transform.trn(dest.x*2,dest.y*2,dest.z*2);
        ...
        for (final StarObjectModelInstance star : spaceMap.getStars()) {
            ...
            for (final GameObjectModelInstance planets : spaceMap.getAllPlanetsFromStar(star)) {
                if (planets.isVisible(perCam)) {
                    modelBatch.render(planets, environment);
                    visibleCount++;
                }
            }
        }
        for (final GameObjectModelInstance ships : spaceMap.getShips()) {
            if (ships.isVisible(perCam)) {
